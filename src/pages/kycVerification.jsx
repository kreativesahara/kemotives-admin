...
/**
 *  Verification Management Component for Admins
 * Allows administrators to review and approve/reject  submissions
 */
const Verification = () => {
  ...
  const fetchRecords = async () => {
    setLoading(true);
    try {
      const response = await axiosPrivate.get(`/api/?status=${filterStatus}`);
  ...
  const handleUpdateStatus = async (newStatus) => {
    ...
    try {
      const response = await axiosPrivate.put(`/api//${selected.id}/status`, {
        status: newStatus,
        notes: reviewNotes,
        adminId: auth.id
      });
      ...
      if (response.status === 200) {
        toast.success(` verification ${newStatus === 'verified' ? 'approved' : 'rejected'} successfully`);
        closeReviewModal();
        fetchRecords();
      } else if (response.status === 403) {
      ...
      <div className="container mx-auto px-4">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow p-6 text-center">
            <p className="text-lg text-gray-600">No  records found with status: {filterStatus}</p>
          </div>
        ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <div className="space-y-2">
                    <p><span className="font-medium">Full Name:</span> {selected.fullName}</p>
                    <p><span className="font-medium">KRA PIN:</span> {selected.kraPin}</p>
                    <p><span className="font-medium">Registration Date:</span> {selected.registrationDate}</p>
                    <p><span className="font-medium">Permit Number:</span> {selected.permitNumber}</p>
                    <p><span className="font-medium">Address:</span> {selected.address}</p>
                    <p><span className="font-medium">Status:</span> {renderStatusBadge(selected.status)}</p>
                    <p><span className="font-medium">Submitted:</span> {new Date(selected.createdAt).toLocaleString()}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="border rounded-lg overflow-hidden mb-4">
                    <img
                      ...
                      title={selected.fullName || 'ID Document'}
                      alt="ID Document"
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    <img
                      ...
                      title={selected.fullName || 'Selfie'}
                      alt="Selfie"
                      className="w-full h-auto object-contain max-h-48"
                    />
                  </div>
