...
function AddAccessories() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosPrivate.post('/api/accessories', form, {
        headers: { 'Content-Type': 'multipart/form-data' },
        withCredentials: true,
      });

      showSuccess("Accessory uploaded successfully");
      navigate(from, { replace: true });
    } catch (error) {
      console.error("Error uploading accessory:", error);
      showError(error.response?.data?.message || "Failed to upload accessory");
    }
  };
        ...
        <div className="max-w-[1024px] mx-auto bg-slate-50 rounded-lg shadow-lg p-8 transition-all duration-300 hover:shadow-xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-3xl font-bold text-gray-800">Add New Accessory</h2>
              <button
                type="submit"
